...
function add_login_url( $items, $args ) {
    ...
    if ( is_user_logged_in() && $args->theme_location == 'header' ) {
      $items .= '<li id="login-tab"><a class="log-in" href="'. get_permalink( get_option('woocommerce_myaccount_page_id') ) .'" title="'.__('My Account','TEXT_DOMAIN').'"><i class="icon-user"></i>'. esc_html( 'My Account', 'TEXT_DOMAIN').'</a></li>';
    }
    /**
     * Else display login menu item.
     */
    elseif ( !is_user_logged_in() && $args->theme_location == 'header' ) {
      $items .= '<li id="login-tab"><a class="log-in" href="'. get_permalink( get_option('woocommerce_myaccount_page_id') ) .'"  title="'.__('Login / Register','TEXT_DOMAIN').'"><i class="icon-user"></i><span>'. esc_html( 'Login/Register', 'TEXT_DOMAIN').'</span></a></li>';
    }
    return $items;
  }
