...
function ats_add_to_cart_message( $message, $products )
{
    ...
    $image    = '';
    if ( $image_id != '' ) {
        $image = '<div class="product-image"><a href="' . $product->get_permalink() . '"><img src="' . wpimage( 'img=' . $image_id . '&w=27&h=27&crop=true&retina=false&upscale=true' ) . '" alt="' . esc_attr( $product->get_title() ) . ' ' . __( 'successfully added to your cart.', 'woocommerce' ) . '"></a></div>';
    }
