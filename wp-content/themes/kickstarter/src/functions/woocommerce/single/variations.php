...
// $is_tax = get_option('woocommerce_tax_display_shop');
        ...
        if ( $_product->get_stock_status() === 'outofstock' || ( $_product->get_manage_stock() === true && $_product->get_stock_quantity() < 1 ) ) {
            $oos = ' - ' . __( 'OUT OF STOCK', 'woocommerce' );
        }

        $tax_tag = get_option( 'woocommerce_price_display_suffix' );

//this is where you can actually customize how the price is displayed
        return $term . ' (' . strip_tags( wc_price( $_product->get_price() ) ) . $tax_tag . ')' . $oos;
    }
    return $term;
