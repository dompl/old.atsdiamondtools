...
function init_ag_epdq()
{
    ...
    class epdq_checkout extends WC_Payment_Gateway
    {
        ...
        public function receipt_page($order_id)
        {
            ...
            $hash_fields = array($settings['pspid'], date('Y:m:d'), $order->get_order_number(), $settings['shain']);
            ...
            foreach ($order_item as $product) {
                $product_name[] = preg_replace("/[^a-zA-Z0-9\s]/", "", str_replace(array("-", " "), "", $product['name'])) . " x" . $product['qty'];
			...
			');
                ...
                echo 'Please double check the ePDQ plugin settings, something is not quite right...';
        ...
        protected function SHA_check($datatocheck)
        {
            ...
            AG_ePDQ_Helpers::ag_log('Debug data sent back ' . print_r($datatocheck, TRUE), 'debug', $this->debug);
        ...
        public function successful_transaction($args)
        {
            ...
            $order_data = array();
            unset($args['SHASIGN'], $args['COMPLUS'], $args['CARDNO']);
            foreach ($args as $key => $value) {
                if ($value == "") {
                    continue;
                }
