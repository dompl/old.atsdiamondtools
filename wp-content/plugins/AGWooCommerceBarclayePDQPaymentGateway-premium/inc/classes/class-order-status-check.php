...
class AG_ePDQ_order_status_check {
	...
	public static function check( $order_id ) {
		...
		$note         = '<p>' . __( 'ePDQ Status:', 'ag_epdq_server' ) . ' - ' . AG_errors::get_epdq_status_code( $status ) . '</p>';
		$errornote    = '';
		$error_note   = '';
		if ( isset( $NCERROR ) ) {
		...
		$note .= '<p>' . __( 'Payment Reference In ePDQ System:', 'ag_epdq_server' ) . ' - ' . $response['PAYID'] . '</p>';
	...
	function ag_manually_check_status_call() {
		...
		$note .= '<p>' . __( 'Order ID:', 'ag_epdq_server' ) . ' - ' . $order->get_id() . '</p>';
		$note .= '<p>' . __( 'Payment Reference In ePDQ System:', 'ag_epdq_server' ) . ' - ' . $PAYID . '</p>';
		...
		if ( in_array( $status, $accepted ) ) {
			...
			AG_ePDQ_Helpers::ag_log( print_r( $status, true ), 'debug', $ePDQ_settings->debug );
