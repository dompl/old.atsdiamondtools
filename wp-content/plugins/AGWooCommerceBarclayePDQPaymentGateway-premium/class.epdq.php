 ...
 * -----
 * Last Modified: Tuesday, 16th October 2018 12:28:59 pm
...
class epdq_checkout extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		if ( ag_epdq_fs()->can_use_premium_code() ) {
	...
	function get_icon() {
		...
		if ( !$this->cardtypes ) {
			...
			foreach ( $this->cardtypes as $cardtype ) {
					$icon .= '<img style="margin-left:5px; margin-right: 5px;" src="' . plugins_url( '/new-card/' . strtolower( $cardtype ) . '.png', __FILE__ ) . '" alt="' . strtolower( $cardtype ) . '" />';
			}
		}
	...
	<?php
		...
		if ( ag_epdq_fs()->can_use_premium_code() ) {
			...
			echo '<br />Please activate your licence';
		} ?>
...
function successfull_transaction($args){
					...
					if (!is_wp_error($result) && $result['response']['code'] >= 200 && $result['response']['code'] < 300) {
					  ...
					  if (in_array($status, $accepted)) {
							$order->add_order_note( __('Refund successful', 'ag_epdq_checkout') . '<br />' .
								__('Refund Amount: ', 'ag_epdq_checkout') . $refund_amount . '<br />' .
								__('Refund Reason: ', 'ag_epdq_checkout') . $reason . '<br />' .
								__('ePDQ Status: ', 'ag_epdq_checkout') . get_epdq_status_code($status) . ' '
						);
					    ...
					    $order->add_order_note( __('Refund failed', 'ag_epdq_checkout') . '<br />' . __('ePDQ Status: ', 'ag_epdq_checkout') . get_epdq_status_code($status) . '<br />' );
							$order->add_order_note( __('Refund failed', 'ag_epdq_checkout') . '<br />' . __('ePDQ Error note: ', 'ag_epdq_checkout') . $string . '<br />' );
							$order->add_order_note( __('Debug note:', 'ag_epdq_checkout') . '<br />Refund only possiable once payment has been confirmed in the ePDQ back office.<br />' );
	...
	public function get_epdq_status_code($code){
		if( $code == '' )
			return;
		$codes =  array(
	...
	public function get_epdq_ncerror($code) {
		if ($code == '')
			return;
...
add_filter('woocommerce_available_payment_gateways', 'epdq_checkout', 99, 1);
function epdq_checkout( $available_gateways ) {
	if (!ag_epdq_fs()->can_use_premium_code()) {
