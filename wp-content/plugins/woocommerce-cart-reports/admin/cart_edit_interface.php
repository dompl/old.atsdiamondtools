...
class AV8_Edit_Interface {
	...
	public function tooltip_scripts() {
		...
		if ( is_admin() ) {
			if ( $pagenow == 'post.php' && get_post_type( get_post( $_GET['post'] ) ) == 'carts' ) {
				...
				wp_enqueue_script( 'moment-duration', 'https://unpkg.com/moment-duration-format@2.2.2/lib/moment-duration-format.js', array( 'moment-js' ) );
	...
	public function cart_status_meta_boxes() {
		add_meta_box( 'cart_status_meta_boxes', __( 'Cart Status', 'woocommerce_cart_reports' ), array(
			$this,
			'cart_status_metabox',
		), 'carts', 'side', 'default' );
	}
	...
	public function cart_action_meta_boxes() {
		add_meta_box( 'cart_action_meta_boxes', __( 'Customer Actions', 'woocommerce_cart_reports' ), array(
			$this,
	...
	public function cart_status_customer_meta_boxes() {
		add_meta_box( 'cart_status_customer_meta_boxes', __( 'Cart Customer', 'woocommerce_cart_reports' ), array(
			$this,
			'cart_status_customer_metabox',
		), 'carts', 'normal', 'default' );
	}
	...
	public function add_cart_items_boxes() {
		...
		add_meta_box( 'woocommerce-cart-items', __( 'Cart Items', 'woocommerce_cart_reports' ), array(
			$this,
	...
	public function cart_useragent_meta_boxes() {
		add_meta_box( 'cart_useragent_meta_boxes', __( 'Cart Data', 'woocommerce_cart_reports' ), array(
			$this,
	...
	public function cart_status_metabox() {
		...
		echo __( '<div><p><strong>Created:</strong><br/>' . date( 'F j, Y \a\t g:i a', $this->receipt->created() ) . '</p></div>' );
	...
	public function cart_useragent_meta_box() {
		...
		echo '<thead>';
		echo '<tr><th class="hist1" width="' . $width_p . '%" style="text-align:left">' . __( 'Cart Last Updated', 'woocommerce_cart_reports' ) . av8_tooltip( __( '<i>Cart Last Updated</i> indicates the last time the customer performed a cart-related action on your site. These actions include viewing the cart, adding new items to the cart, updating quantities, or removing products from the cart.', 'woocommerce_cart_reports' ), false ) . '</th>
';          //If the cart is converted, show how long it spent as abandoned/open
		if ( $this->receipt->status() == 'Converted' ) {
			echo '<th class="ip" width="' . $width_p . '%" style="text-align:left;">' . __( 'Time To Conversion', 'woocommerce_cart_reports' ) . av8_tooltip( __( '<i>Time to Conversion</i> indicates total time elapsed from when the cart was first created until the actual conversion. (purchase)', 'woocommerce_cart_reports' ), false ) . '</th>';
		} else {
			echo '<th class="ip" width="' . $width_p . '%" style="text-align:left;">' . __( 'Cart Age', 'woocommerce_cart_reports' ) . av8_tooltip( __( '<i>Cart Age</i> indicates the total time elapsed since this non-converted cart has been created.', 'woocommerce_cart_reports' ), false ) . '</th>';
		}
		...
		if ( $this->receipt->status() == 'Converted' ) {
			...
			<script type='text/javascript'>
				...
				DaysHMSCounter.prototype.update = function() {
					this.calculate();
					this.container.innerHTML = '<p>' + this.duration.format( '[<strong>]d[</strong>] __ [<strong>]h[</strong>] _ [<strong>]m[</strong>] _ [<strong>]s[</strong>] _' ) + '</p>';
					var self = this;
					setTimeout( function() {
						self.update();
					}, (
		...
		<div class="woocommerce_cart_reports_items_wrapper">
			...
			<table cellpadding="0" width="100%" cellspacing="0" class="woocommerce_cart_reports_items">
				...
				if ( count( $order_items ) > 0 ) {
					foreach ( $order_items as $item ) :
						...
						<tr class="item <?php echo $table_color; ?>" rel="<?php echo $loop; ?>">
							<td class="thumb">
								...
								echo '<strong>' . __( 'Product ID:', 'woocommerce_cart_reports' ) . '</strong> ' . $_product->get_id();
							...
							<td class="sku">
								...
								<input type="hidden" class="item_id" name="item_id[<?php echo $loop; ?>]"
								       value="<?php if ( isset( $item->id ) && $item->id != '' ) {
					...
					<?php av8_tooltip( __( 'When a customer adds
