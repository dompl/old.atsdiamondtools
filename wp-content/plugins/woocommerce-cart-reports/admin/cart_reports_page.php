	...
	if ( $chosen_product_ids && is_array( $chosen_product_ids ) ) {
		...
		<table class="bar_chart">
			<thead>
				...
				<tbody>
				<?php
					if (count($product_sales)>0) foreach ($product_sales as $date => $sales) :
...
?>
				...
				<div class="inside">
					<p class="stat"><?php count($total_converted);  if($total_converted != '') echo $num_converted . ' Carts' ; else _e('n/a', 'woocommerce'); ?></p>
				</div>
			</div>
