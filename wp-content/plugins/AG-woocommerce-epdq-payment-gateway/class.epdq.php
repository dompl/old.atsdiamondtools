...
if ( ! function_exists( 'ag_plugin_licence' ) ) {
...
if ( ! function_exists( 'AG_plugins_page' ) ) {

	function AG_plugins_page() {
	    echo '<div class="wrap">';
	    echo '<h2>We are AG Plugin License Management</h2>';
			...
			do_action( 'load_ag_plugins' );

			echo '<p>When moving plugin from one site to another (from development server to live site) you <strong>MUST</strong> deactivate your plugin first on development before you move it over.</p>';
	    echo '</div>';
	}
...
add_action( 'load_ag_plugins', 'epdq_licence_setting' );
function epdq_licence_setting() {

	/*** License activate button was clicked ***/
	if (isset($_REQUEST['PS_ePDQ_key_activate_license'])) {
		$license_key = $_REQUEST['PS_ePDQ_key'];
		...
		$api_params = array(
			...
			'item_reference' => urlencode(ITEM),
			'product_id' => '1283, 2132, 2133, 2134',
		);
		...
		if($license_data->result == 'success'){//Success was returned for the license activation
				update_option('PS_ePDQ_key', $license_key);
				update_option('PS_ePDQ_key_message', $license_data->message);
				add_option('PS_ePDQ_key_message', $license_data->message);
				$fulldate = $response['headers']['expires'];
				$words = explode(" ", $fulldate);
				$date = join(" ", array_slice($words, 0, 4));
				add_option('PS_ePDQ_key_expires', $date);
				update_option('PS_ePDQ_key_expires', $date);
		} else {
			update_option('PS_ePDQ_key_message', $license_data->message);
		}
	...
	/*** License activate button was clicked ***/
	if (isset($_REQUEST['PS_ePDQ_key_deactivate_license'])) {
		$license_key = $_REQUEST['PS_ePDQ_key'];
		...
		if($license_data->result == 'success'){//Success was returned for the license activation
				...
				update_option('PS_ePDQ_key', '');
				update_option('PS_ePDQ_key_message', $license_data->message);
				echo $response->headers->expires;
		} else {
			update_option('PS_ePDQ_key_message', $license_data->message);
		}
...
?>
<form action="" method="post" style="padding-top: 20px; padding-bottom: 20px;">
	<table class="form-table">
		<tr><br  />
      <label class="licence_label" for="PS_ePDQ_key"><strong>ePDQ Licence Key</strong></label>
      <th style="width:10px;  padding-bottom: 0;"><input class="regular-text" type="password" id="PS_ePDQ_key" name="PS_ePDQ_key"  value="<?php echo get_option('PS_ePDQ_key'); ?>" ><input type="submit" name="PS_ePDQ_key_activate_license" value="Activate" class="button-primary activate_license" /><input type="submit" name="PS_ePDQ_key_deactivate_license" value="Deactivate" class="button" />
				<?php if (get_option('PS_ePDQ_key_message') == 'License key activated') {
					echo '<p style="color: green"><strong>'. get_option('AGD_SagePay_Key_message'). '</strong></p>';
				} elseif (get_option('PS_ePDQ_key_message') == 'Your License key has expired') {
					echo '<p style="color: red"><strong>'. get_option('PS_ePDQ_key_message'). ' to continue getting support and updates make sure you <a href="">update your licence here</a> now.</strong></p>';
				} else {
					echo '<p style="color: red"><strong>'. get_option('PS_ePDQ_key_message'). '</strong></p>';
				} ?>
  <br  /></tr>
	</table>
	<p style="margin-top: 0; ">Need some help setting up this plugin? <a href="<?php echo site_url(); ?>/wp-admin/index.php?page=welcome-screen-about">Click here</a></p>
</form>
<hr />
...
public function __construct() {
		...
		$this->icon 		= plugins_url('AG-woocommerce-epdq-payment-gateway/cards.gif');
		$this->has_fields 	= false;

		$license = get_option('PS_ePDQ_key');
		$api_params = array(
		...
		add_action( 'woocommerce_api_woocommerce_epdq_checkout', array( $this, 'successful_request' ) );
	...
	<?php
			// If the user said yes to showcase
			// Website information
			$websitename = get_option( 'blogname' );
			$websiteurl = get_option ('siteurl');
			$admin_email = get_option( 'admin_email' );
			$showcasedata = array('Website name:', $websitename, 'Website URL: ', $websiteurl, 'Email: ', $admin_email,);
			$showcase = implode("\n", $showcasedata);

			if ($this->showcase == 'yes') {
				// Send the email to support only once the user has ticked that they are happy.
				$to = 'support@weareag.co.uk';
				$subject = 'Epdq plugin showcase';
				$body = $showcase;
			...
			$themename = $theme_data->display( 'Name', false, false );
			$themeURL = $theme_data->display( 'ThemeURI', false, false );
			$themeversion = $theme_data->display( 'Version', false, false );

			// Website information
			$websitename = get_option( 'blogname' );
			$websiteurl = get_option ('siteurl');

			// Add all data to array ready for email
			$emaildata = array('Theme name:', $themename, 'Theme URL', $themeURL, 'Theme Version:', $themeversion, 'Website name:', $websitename, 'Website URL: ', $websiteurl);
			$emailcleandata = implode("\n", $emaildata);

			if ($this->debug == 'yes') {
				// Send the email to support only once the user has ticked that they are happy.
				$to = 'support@weareag.co.uk';
				$subject = 'Epdq debug email';
				$body = $emailcleandata;
				$headers = array('Content-Type: text/html; charset=UTF-8');
				wp_mail( $to, $subject, $body, $headers );
			}
		...
		echo '</strong>';
		if($license_data->result == 'success'){
			echo '<br />Thank You Purchasing!';
		} else {
			echo '<br />Please activate your licence <a href="'. site_url() .'/wp-admin/admin.php?page=AG_licence">here</a>';
		} ?>
			<table class="form-table">
				<?php $this->generate_settings_html(); ?>
			</table> <!--/.form-table-->

<p>Need some help setting up this plugin? <a href="<?php echo site_url(); ?>/wp-admin/index.php?page=welcome-screen-about">Click here</a></p>
		...
		public function init_form_fields() {
			$this->form_fields = array(
					...
					'logo' => array(
						'title' => __( 'Logo for payment page', 'woocommerce' ),
						'type' => 'text',
						'label' => __( 'Upload image for payment page', 'woocommerce' ),
						'default' => '',
					...
					'debug' => array(
							'title' => __( 'Allow anonymous tracking.', 'woocommerce' ),
							'type' => 'checkbox',
							'label' => __( 'Enable plugin debug,', 'woocommerce' ),
							'default' => '',
							'description'=>'Please allow our plugin to track anonymous data, so that we can strongly improve our plugin and your user experience. This will *not* slow down your website or allow us access to your website. ',
							'desc_tip'=>true
						),
					'refund' => array(
						'title' => __( 'Refunds', 'woocommerce' ),
						'type' => 'checkbox',
						'label' => __( 'Process refunds in order screen', 'woocommerce' ),
						'default' => '',
					...
					'prefix' => array(
						'title' => __( 'Add a prefix to your order numbers', 'woocommerce' ),
						'type' => 'text',
						'label' => __( 'Add a prefix to your order numbers', 'woocommerce' ),
						'default' => '',
						'desc_tip'=>true
					),
	...
	function receipt_page($order_id){
		...
		$this->notify_url = str_replace( 'https:', 'http:', add_query_arg( 'wc-api', 'epdq_checkout', $order->get_checkout_order_received_url() ) );
	...
	//	}
		...
		wc_enqueue_js('
			jQuery("body").block({
					message: "'.__('You are now redirecting to Barclays server to make payment securly.', 'ag-epdq').'",
					overlayCSS:
					{
						background: "#fff",
						opacity: 0.6
	...
	function thank_you($order_id) {
		...
		$order = new WC_Order( $order_id );
				...
				$dienote .= '<br>Error Code: ' . $ncerror . ' - ' . $this->get_epdq_ncerror($ncerror) . '</p>';
				$died = '';
				$died .= $dienote;
				...
				if (in_array($STATUS, $accepted)) {
						...
						if (!empty($_REQUEST['ORDERID']))
								$note = 'Order ID: ' . $_REQUEST['ORDERID'] . '.<br><br>'; //	order id
						if (!empty($_REQUEST['AMOUNT']))
								$note .= 'Amount: ' . $_REQUEST['AMOUNT'] . '.<br><br>'; //	amount
						if (!empty($_REQUEST['CURRENCY']))
								$note .= 'Order currency: ' . $_REQUEST['CURRENCY'] . '.<br><br>'; //	order currency
						if (!empty($_REQUEST['PM']))
								$note .= 'Payment Method: ' . $_REQUEST['PM'] . '.<br><br>'; //	payment method
						if (!empty($_REQUEST['ACCEPTANCE']))
								$note .= 'Acceptance code returned by acquirer: ' . $_REQUEST['ACCEPTANCE'] . '.<br><br>';    //	acceptance
						if (!empty($_REQUEST['STATUS']))
								$note .= 'Transaction status : ' . $_REQUEST['STATUS'] . '.<br><br>'; //	status code
						if (!empty($_REQUEST['PAYID']))
								$note .= 'Payment reference in EPDQ system: ' . $_REQUEST['PAYID'] . '.<br><br>'; //	pay id
						if (!empty($_REQUEST['NCERROR']))
								$note .= 'Error Code: ' . $_REQUEST['NCERROR'] . '.<br><br>'; //	ncerror
						if (!empty($_REQUEST['BRAND']))
								$note .= 'Card brand (EPDQ system derives this from the card number) : ' . $_REQUEST['BRAND'] . '.<br><br>'; //	brand

						if (!empty($_REQUEST['TRXDATE']))
								$note .= 'Transaction Date: ' . $_REQUEST['TRXDATE'] . '.<br><br>'; //	date
						if (!empty($_REQUEST['CN']))
								$note .= 'Cardholder/customer name: ' . $_REQUEST['CN'] . '.<br><br>'; //	payer's name
						if (!empty($_REQUEST['IP']))
								$note .= 'Customer\'s IP: ' . $_REQUEST['IP'] . '.<br><br>'; //	payer's ip

						if (!empty($_REQUEST['AAVADDRESS']))
								$note .= 'AAV result for the address: ' . $_REQUEST['AAVADDRESS'] . ' . <br><br>'; //	aav address
						if (!empty($_REQUEST['AAVCHECK']))
								$note .= 'Result of the automatic address verification: ' . $_REQUEST['AAVCHECK'] . ' . <br><br>'; //	aav check
						if (!empty($_REQUEST['AAVZIP']))
								$note .= 'AAV result for the zip code: ' . $_REQUEST['AAVZIP'] . ' . <br><br>'; // aav zip

						if (!empty($_REQUEST['CCCTY']))
								$note .= 'Country where the card was issued: ' . $_REQUEST['CCCTY'] . ' . <br><br>';
						if (!empty($_REQUEST['COMPLUS']))
								$note .= 'Custom value passed: ' . $_REQUEST['COMPLUS'] . ' . <br><br>';

						if (!empty($_REQUEST['CVCCHECK']))
								$note .= 'Result of the card verification code check: ' . $_REQUEST['CVCCHECK'] . ' . <br><br>';
						if (!empty($_REQUEST['ECI']))
								$note .= 'Electronic Commerce Indicator: ' . $_REQUEST['ECI'] . ' . <br><br>';
						if (!empty($_REQUEST['FXAMOUNT']))
								$note .= 'FXAMOUNT: ' . $_REQUEST['FXAMOUNT'] . ' . <br><br>';
						if (!empty($_REQUEST['FXCURRENCY']))
								$note .= 'FXCURRENCY: ' . $_REQUEST['FXCURRENCY'] . ' . <br><br>';
						if (!empty($_REQUEST['IPCTY']))
								$note .= 'Originating country of the IP address: ' . $_REQUEST['IPCTY'] . ' . <br><br>';
						if (!empty($_REQUEST['SUBBRAND']))
								$note .= 'SUBBRAND: ' . $_REQUEST['SUBBRAND'] . ' . <br><br>';
						if (!empty($_REQUEST['VC']))
								$note .= 'Virtual Card type: ' . $_REQUEST['VC'] . ' . <br><br>';

								// Store these for later
								if (isset($_REQUEST['ORDERID'])) {
									update_post_meta($order->id, 'OrderID', $_REQUEST['ORDERID']);
								}
								if (isset($_REQUEST['AMOUNT'])) {
									update_post_meta($order->id, 'OrderAmount', $_REQUEST['AMOUNT']);
								}
								if (isset($_REQUEST['CURRENCY'])) {
									update_post_meta($order->id, 'OrderCurrency', $_REQUEST['CURRENCY']);
								}
								if (isset($_REQUEST['PM'])) {
									update_post_meta($order->id, 'PaymentMethod', $_REQUEST['PM']);
								}
								if (isset($_REQUEST['ACCEPTANCE'])) {
									update_post_meta($order->id, 'Acceptance', $_REQUEST['ACCEPTANCE']);
								}
								if (isset($_REQUEST['STATUS'])) {
									update_post_meta($order->id, 'Status', $_REQUEST['STATUS']);
								}
								if (isset($_REQUEST['PAYID'])) {
									update_post_meta($order->id, 'PAYID', $_REQUEST['PAYID']);
								}
								if (isset($_REQUEST['NCERROR'])) {
									update_post_meta($order->id, 'NCERROR', $_REQUEST['NCERROR']);
								}
								if (isset($_REQUEST['BRAND'])) {
									update_post_meta($order->id, 'BRAND', $_REQUEST['BRAND']);
								}
								if (isset($_REQUEST['TRXDATE'])) {
									update_post_meta($order->id, 'TRXDATE', $_REQUEST['TRXDATE']);
								}
								if (isset($_REQUEST['IP'])) {
									update_post_meta($order->id, 'IP', $_REQUEST['IP']);
								}
								update_post_meta( $order->id, '_transaction_id', $_REQUEST['PAYID'] );
		...
		function process_refund( $order_id, $amount = NULL, $reason = '' ) {
					...
					if (!is_wp_error($result) && $result['response']['code'] >= 200 && $result['response']['code'] < 300) {
					  ...
					  if (in_array($status, $accepted)) {
							$order->add_order_note( __('Refund successful', 'ag-sagepay-direct') . '<br />' .
								__('Refund Amount: ', 'ag-sagepay-direct') . $amount . '<br />' .
								__('Refund Reason: ', 'ag-sagepay-direct') . $reason . '<br />' .
								__('ePDQ Status: ', 'ag-sagepay-direct') . get_epdq_status_code($status) . ' '
						);
					    ...
					    $order->add_order_note( __('Refund failed', 'ag-sagepay-direct') . '<br />' . __('ePDQ Status: ', 'ag-sagepay-direct') . get_epdq_status_code($status) . '<br />' );
							$order->add_order_note( __('Refund failed', 'ag-sagepay-direct') . '<br />' . __('ePDQ Error note: ', 'ag-sagepay-direct') . $string . '<br />' );
							$order->add_order_note( __('Debug note:', 'ag-sagepay-direct') . '<br />Refund only possiable once payment has been confirmed in the ePDQ back office.<br />' );
