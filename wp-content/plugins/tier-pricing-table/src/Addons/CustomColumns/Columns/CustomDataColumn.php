...
class CustomDataColumn extends AbstractCustomColumn {
		...
		if ( empty( $percentageValues ) || empty( $fixedValues ) ) {
			...
			if ( $isProductRule ) {
				...
				if ( $product->get_parent_id() ) {
					$percentageValues = $percentageValues ? $percentageValues : $this->getValue( $product->get_parent_id(),
							'percentage', $role );
					$fixedValues      = $fixedValues ? $fixedValues : $this->getValue( $product->get_parent_id(),
							'fixed', $role );
				}
			}
		}
	...
	public function renderInputField(
		...
		?>
		<input type="<?php echo esc_attr( $this->getDataType() === 'number' ) ? 'number' : 'text'; ?>"
